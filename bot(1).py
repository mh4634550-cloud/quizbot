...
async def safe_reply(message_or_query, text: str, reply_markup=None, parse_mode="Markdown"):
    ...
    try:
        ...
        logger.info("Markdown send failed; retrying as plain text.")
        try:
...
def parse_with_gemini(part: types.Part):
    ...
    for model_name in FALLBACK_MODELS:
        try:
            ...
            last_error = e
            logger.info(f"Model {model_name} unavailable; trying fallback.")
            continue
...
async def send_next_poll(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not questions or index >= len(questions):
        ...
        try:
            ...
            logger.info("Summary Markdown send failed; sending plain text.")
...
def main():
    ...
    logger.info("Ultra-Fast QuizBot & GK Assistant Running...")
