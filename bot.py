    ...
    for b in re.split(r"\n\s*\n|(?=\n\s*(?:Q\s*\d+[\.\)]|\d+[\.\)]))", text):
        ...
        for l in lines[1:]:
            if re.match(r"^[\(\[]?[A-Da-d1-4][\)\].]", l):
                ...
                if m: c_idx = {"A":0,"1":0,"B":1,"2":1,"C":2,"3":2,"D":3,"4":3}.get(m.group(1).upper(), 0)
        if len(opts) >= 2:
            out.append({"question": q_txt[:280], "options": opts[:4], "correct_id": min(c_idx, len(opts)-1)})
    return out

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 **Dulhin Bazar Quiz Bot**\n\n🔹 `/create_quiz` - Banayein Quiz (Photo/Text)\n🔹 `/store` - Quiz Store\n🔹 `/stop` - Running Quiz Rokein", parse_mode="Markdown")
...
async def finalize_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid not in creation_state or not creation_state[uid]["questions"]:
        return await update.message.reply_text("⚠️ Questions upload karein pehle!")
...
async def store(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not db.get("quizzes"): return await update.message.reply_text("Store khali hai. `/create_quiz` karein.")
    kb = [[InlineKeyboardButton(f"{q['title']} - {'FREE' if q['price']==0 else f'₹{q[\"price\"]}'}", callback_data=f"view_{qid}")] for qid, q in db["quizzes"].items()]
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("p_"):
        creation_state[uid]["price"], creation_state[uid]["step"] = int(d.split("_")[1]), "QUESTIONS"
        ...
        kb = [[InlineKeyboardButton("I am ready!", callback_data=f"start_{qid}")]] if has else [[InlineKeyboardButton(f"💳 Buy Now (₹{qz['price']})", callback_data=f"buy_{qid}")]]
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for s in active_sessions.values():
        if ans.poll_id in s["map"]:
            ...
            if ans.option_ids and ans.option_ids[0] == s["map"][ans.poll_id]: u["c"] += 1
            else: u["w"] += 1
...
async def stop_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    else: await update.message.reply_text("Koi active quiz nahi hai.")
...
async def main():
    ...
    for c, h in [("start",start),("create_quiz",create_quiz),("done",finalize_quiz),("store",store),("stop",stop_quiz)]: app.add_handler(CommandHandler(c, h))
